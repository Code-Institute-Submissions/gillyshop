...
- One issue I found when testing is with the payment form. The payment form has the csv/cvv as not required as this makes it more secure and Stripe is meant to handle this side of the payment, however the form still submits when this is not filled in. I did some research and found that Maestro cards don't have CVVs so this may be the reason that Stripe allows it to go through. I left the cvv field as not required to prevent any security issues.
